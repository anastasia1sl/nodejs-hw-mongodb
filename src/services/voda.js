import { VodaCollection } from '../db/models/voda.js';

export const createWater = async (payload) => {
  let { amount, date, userId } = payload;

  const water = await VodaCollection.create({
    amount,
    date: new Date(date),
    owner: userId,
  });
...
export const getWaterById = async (waterId, userId) => {
  const water = await VodaCollection.findOne({
    _id: waterId,
    owner: userId,
  });
...
export const updateWaterById = async (
  ...
  const rawResult = await VodaCollection.findOneAndUpdate(
...
export const deleteWaterById = async (waterId, userId) => {
  const water = await VodaCollection.findOneAndDelete({
    _id: waterId,
    owner: userId,
  });
...
export const getWaterPerDay = async ({ userId, date }) => {
  ...
  const waterRecords = await VodaCollection.find({
    owner: userId,
